<template>
  ...
  <div class="p-4">
    <div v-if="selectedTab === 'Eventos'">
      ...
      <h2 class="text-xl">Completadas</h2>
      <div class="m-2 max-w-sm p-4 border-b border-slate-300 items-center flex flex-wrap" v-for="event in completed" v-bind:key="event._id">
        <div class="basis-1/6">
          <img src="@/assets/icons/calendar.svg" width="15" alt="">
        </div>
        <div class="basis-3/6 text-left">
...
export default {
  ...
  async mounted() {
    ...
    response = await this.callEndpoints(this.axios, 'get', '/activity?status=TODO', localStorage.token);
    this.events = response.data;

    response = await this.callEndpoints(this.axios, 'get', '/activity?status=COMPLETED', localStorage.token);
